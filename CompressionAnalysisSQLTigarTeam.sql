...
AS (SELECT tr.objname,
           ...
           (tr.sample_req * 100) / CASE
                                       WHEN tr.sample_cur = 0 THEN
                                           1
                                       ELSE
                                           tr.sample_cur
                                   END AS pct_of_orig_row,
           (tp.sample_req * 100) / CASE
                                       WHEN tp.sample_cur = 0 THEN
                                           1
                                       ELSE
                                           tp.sample_cur
...
AS (SELECT Table_Name,
           ...
           CASE
               WHEN [ROW_estimate_Pct_of_orig] >= 100
                    AND [PAGE_estimate_Pct_of_orig] >= 100 THEN
                   'NO_GAIN'
               WHEN [Percent_Update] >= 10 THEN
                   'ROW'
               WHEN [Percent_Scan] <= 1
                    AND [Percent_Update] <= 1
                    AND [ROW_estimate_Pct_of_orig] < [PAGE_estimate_Pct_of_orig] THEN
                   'ROW'
               WHEN [Percent_Scan] <= 1
                    AND [Percent_Update] <= 1
                    AND [ROW_estimate_Pct_of_orig] > [PAGE_estimate_Pct_of_orig] THEN
...
SELECT @StartDate AS Collecting_Data_Since,
       CONVERT(VARCHAR(4), @UpTime / 60 / 24) + 'd ' + CONVERT(VARCHAR(4), @UpTime / 60 % 24) + 'h '
       + CONVERT(VARCHAR(4), @UpTime % 60) + 'm' AS Collecting_Data_For;
